"""Save and verify the three attention-grabbing Chuseok posts (text-only, no images)."""
import hashlib
import json
import re
import time
...
FILES = sorted((ROOT / "drafts").glob("20260923-chuseok-attention-*.md"))
RECEIPT = ROOT / "drafts/20260923-chuseok-attention-save-receipts.json"
...
def prepare(path):
    ...
    assert 20 <= len(tags) <= 30, (path.name, len(tags))
    assert not re.search(r"[｜·|()]", title), title
    assert len(body) >= 2000, (path.name, len(body))
